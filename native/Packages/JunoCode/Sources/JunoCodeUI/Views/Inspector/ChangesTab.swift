...
struct ChangesTab: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Revert all unreviewed files?", isPresented: $confirmsRevertAll, titleVisibility: .visible) {
    ...
    private var changeSummary: some View {
        ...
        .accessibilityLabel("\(PathDisplay.fileCount(controller.changes.count)), \(totalAdded) added, \(totalRemoved) removed")
    }
    ...
    private var actions: some View {
        if revertingAll { ProgressView().controlSize(.small) }
        Button("Revert All") { confirmsRevertAll = true }
            .controlSize(.small)
            .frame(minWidth: 44, minHeight: 44)
            .contentShape(.rect)
            .disabled(revertingAll || !isEditable)
            .help(isEditable ? "Restore every changed file from its checkpoint" : "Ask and Plan sessions are read-only")
            .accessibilityIdentifier("juno.code.changes.revert-all")

        Button("Keep All") { controller.acceptAll() }
            .controlSize(.small)
    ...
    private var isEditable: Bool { controller.session.configuration.behavior == .code }
    ...
    private func row(_ change: TrackedChange) -> some View {
        HStack(spacing: JunoSpace.snug) {
            ReviewStateGlyph(state: change.reviewState).frame(width: 16)
            VStack(alignment: .leading, spacing: 1) {
                Text(PathDisplay.fileName(change.path)).junoCode().lineLimit(1)
        ...
        .accessibilityElement(children: .combine)
        .accessibilityLabel("\(change.path), \(change.kind.rawValue), \(change.linesAdded) added, \(change.linesRemoved) removed")
        ...
        .contextMenu {
            ...
            Button("Revert", role: .destructive) { pendingRevertPath = change.path }
                .disabled(!isEditable)
        }
    }
    ...
    private func revertFile(_ path: String, force: Bool) {
        Task {
            let result = await review.revertFile(path, force: force, using: controller)
            switch result {
            case .restored: break
            case .diverged where !force: pendingForcedRevertPath = path
            case .diverged:
                revertFailureTitle = "Could Not Revert File"
                revertFailureMessage = result.failureMessage ?? "The file still differs from its checkpoint."
    ...
    private func subtitle(_ change: TrackedChange) -> String {
        guard let directory = PathDisplay.directory(change.path) else { return change.kind.rawValue }
