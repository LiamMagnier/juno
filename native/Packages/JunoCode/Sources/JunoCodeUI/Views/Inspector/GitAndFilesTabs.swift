    ...
    var body: some View {
        List {
            ...
            Section("Session") {
                ...
                LabeledContent(
                    "Reasoning",
                    // Absent is a real answer, not a missing one: `nil` means the
                    // turn sent no thinking parameter at all — the reader chose
                    // Instant, or the model publishes no depth control. Rendering
                    // it as "Instant" states that; force-unwrapping would crash on
                    // exactly the sessions that chose it.
                    value: controller.session.configuration.reasoningEffort?.rawValue.capitalized ?? "Instant"
                )
                LabeledContent(
                    "Role",
