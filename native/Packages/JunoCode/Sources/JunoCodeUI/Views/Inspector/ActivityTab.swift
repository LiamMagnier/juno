...
struct ActivityTab: View {
    ...
    /// `CodeToolDigest` gives the transcript's working row — rather than from a
    /// proposal that may still be waiting for permission.
    ...
    private var statusDetail: String {
        ...
        case .planning:
            return "Building the implementation approach"
        case .queued:
        ...
        case .degraded:
            return "Continuing with reduced capability"
        case .needsApproval:
            return "A decision is required before work can continue"
        case .stopping:
            return "Finishing the current operation before stopping"
        case .finished:
            return "Task completed"
        case .failed:
            return "The run stopped because something went wrong"
        case .stopped:
            return "The run was stopped"
        case .hostOffline:
            return "The computer running this session is offline"
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Screen control is a live capability, not historical activity. Its
            // stop action stays outside the scrollable list so it can never be
            // pushed off-screen while Juno is driving the pointer.
    ...
    private var overview: some View {
        List {
            ...
            Section("Work produced") {
                metricRow(
                    label: "Changed files",
                    value: "\(controller.changes.count)",
                    symbol: "doc.badge.ellipsis"
                )
                metricRow(
                    label: "Restorable versions",
            ...
            Section("Session") {
                ...
                LabeledContent(
                    "Mode",
                    value: String(describing: controller.session.configuration.behavior).capitalized
                )
                .accessibilityIdentifier("juno.code.activity.mode")
            }

            // Consent, TCC grants, captured display and latest capture remain in
            // the same overview because Computer Use is part of what the run is
            // doing to the machine, not a hidden settings concern.
    ...
    private var currentWorkRow: some View {
        if let runningTool {
            ...
            LabeledContent(
                "Current work",
                value: "Delegated to \(activeAgents) agent\(activeAgents == 1 ? "" : "s")"
            )
            .accessibilityIdentifier("juno.code.activity.running")
        } else {
    ...
    private func approvalRow(
        _ blockingApproval: (request: ApprovalRequest, others: Int)
    ) -> some View {
        LabeledContent {
            VStack(alignment: .trailing, spacing: JunoSpace.hairline) {
        ...
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            "Approval required: \(blockingApproval.request.toolName), \(blockingApproval.request.summary)"
        )
