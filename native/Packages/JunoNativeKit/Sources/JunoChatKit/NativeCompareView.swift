...
public struct NativeCompareView: View {
    ...
    /// One control, two jobs — the same morph the composer uses everywhere else
    /// in Juno. Two separate buttons would leave a dead Send sitting beside Stop
    /// for the whole run.
    private var sendControl: some View {
        Button {
            if model.anyStreaming { model.stopAll() } else { send() }
        } label: {
...
struct NativeComparePane: View {
    /// The model itself rather than a bag of closures. Closures stored on a
    /// `View` have to be `@Sendable` under strict concurrency, and every one of
    /// them here would capture this same main-actor object anyway.
    @Bindable var model: NativeCompareModel
    ...
    private var run: NativeCompareModel.Run {
        model.runs[pane.id] ?? NativeCompareModel.Run()
    }

    private var modelBinding: Binding<String> {
        Binding(
            get: { pane.modelID },
            set: { [paneID = pane.id, model] newValue in model.setModel(newValue, for: paneID) }
        )
    }
    ...
    private var header: some View {
        HStack(spacing: JunoSpace.tight) {
            ...
            Spacer(minLength: 0)
            if model.canRemovePane {
                Button { model.removePane(pane.id) } label: {
    ...
    private var answer: some View {
        ...
        } else if run.status == .error, run.content.isEmpty {
            failure
        } else if run.content.isEmpty {
            thinking
        } else {
    ...
    private var failure: some View {
        VStack(alignment: .leading, spacing: JunoSpace.snug) {
            ...
            if run.errorAction == .retry {
                Button { model.retry(pane.id) } label: {
