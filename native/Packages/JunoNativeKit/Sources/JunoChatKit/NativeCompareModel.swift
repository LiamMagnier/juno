...
public final class NativeCompareModel {
    ...
    /// Panes the reader has asked to stop. A stream that ends without a terminal
    /// frame means two very different things depending on this: a dropped
    /// connection ("run it again") or a granted Stop ("you stopped it"), and
    /// telling the reader the first when they pressed the second is a lie about
    /// their own action.
    ...
    public func stopAll() {
        ...
        stopping = true
        stopRequested.formUnion(tasks.keys)
    ...
    private func consume(
        ...
        do {
            ...
            if !sawTerminal {
                if Task.isCancelled || stopRequested.contains(paneID) {
