...
final class NativeCompareModelTests: XCTestCase {
    ...
    func testAPaneIsStreamingUntilItsOwnRunFinishes() async throws {
        ...
        // Panes fan out concurrently, so `requests[0]` is not necessarily
        // `panes[0]`. Resolving by model is the only stable pairing — assuming
        // otherwise made these tests fail on whichever pane won the race.
        let first = pane(model, forModel: client.requests[0].modelID)
        client.emit(.textDelta("half"), to: client.requests[0].generationID)
        await settle(until: { model.runs[first]?.content == "half" })
        ...
        await settle(until: { !model.anyStreaming })
    ...
    func testTheFinalMessageReplacesTheStreamedDeltas() async throws {
        ...
        let target = pane(model, forModel: client.requests[0].modelID)
        ...
        await settle(until: { self.settled(model, target) })
        XCTAssertEqual(model.runs[target]?.content, "the real answer")
    }
    ...
    func testFramesFromASupersededRunAreDropped() async throws {
        ...
        model.setModel("c:three", for: target)
        await client.waitForRequests(3)
        XCTAssertEqual(model.panes.first { $0.id == target }?.modelID, "c:three")
        ...
        await settle(until: { model.runs[target]?.content == "from the new model" })

        XCTAssertEqual(model.runs[target]?.content, "from the new model")
    }
    ...
    func testChangingAModelBeforeAnyPromptOnlyResetsThePane() async throws {
        ...
        await settle(until: { model.runs[model.panes[0].id]?.status == .idle })
    ...
    func testStopCancelsEveryInFlightGenerationServerSide() async throws {
        ...
        // Stopping clears itself the moment the last pane settles; a button that
        // stayed disabled after everything had stopped would read as stuck.
        XCTAssertFalse(model.stopping)
        XCTAssertFalse(model.anyStreaming)
    }

    /// A stop granted by a server that closes without a terminal frame must read
    /// as "you stopped this", not as "the connection dropped, run it again". The
    /// reader knows which of those they did.
    func testAStoppedPaneKeepsWhatStreamedAndSaysWhoStoppedIt() async throws {
        let client = CompareClient()
        let model = makeModel(client)
        model.submit("go")
        await client.waitForRequests(2)
        let stopped = pane(model, forModel: client.requests[0].modelID)
        client.emit(.textDelta("as far as it got"), to: client.requests[0].generationID)
        await settle(until: { model.runs[stopped]?.content == "as far as it got" })

        model.stopAll()
        await settle(until: { self.settled(model, stopped) })

        let run = model.runs[stopped]
        XCTAssertEqual(run?.status, .done)
        XCTAssertEqual(run?.content, "as far as it got")
        XCTAssertEqual(run?.finishReason, .userStopped)
    }
    ...
    func testAFailedFrameLeavesThePaneRetryable() async throws {
        ...
        let failing = pane(model, forModel: client.requests[0].modelID)
        await settle(until: { self.settled(model, failing) })

        let run = model.runs[failing]
    ...
    func testAStreamThatEndsWithoutATerminalFrameIsAnError() async throws {
        ...
        let dropped = pane(model, forModel: client.requests[0].modelID)
        client.emit(.textDelta("partial"), to: client.requests[0].generationID)
        client.close(client.requests[0].generationID)
        await settle(until: { self.settled(model, dropped) })

        let run = model.runs[dropped]
        XCTAssertEqual(run?.status, .error)
    ...
    func testTheReceiptComesFromTheServerAndIsNotInventedWhenAbsent() async throws {
        ...
        // Panes fan out concurrently, so request order is NOT pane order — the
        // pane is found by the model it was asked to run.
        let pricedPane = pane(model, forModel: client.requests[0].modelID)
        let unpricedPane = pane(model, forModel: client.requests[1].modelID)
        client.finish(
        ...
        let priced = model.runs[pricedPane]
        ...
        let unpriced = model.runs[unpricedPane]
    ...
    func testCostFallsBackToTheManifestPriceWhenTheServerSendsNone() async throws {
        ...
        let paneID = pane(model, forModel: client.requests[0].modelID)
        await settle(until: { self.settled(model, paneID) })
        XCTAssertEqual(model.runs[paneID]?.costUsd ?? 0, 18, accuracy: 0.0001)
    }

    // MARK: - Helpers

    /// The pane running a given model. Panes fan out concurrently, so the order
    /// requests arrive in is not the order the panes are in.
    private func pane(_ model: NativeCompareModel, forModel modelID: String) -> String {
        model.panes.first { $0.modelID == modelID }?.id ?? ""
    }

    /// Waits for a condition rather than for a fixed number of yields.
    ///
    /// The model owns its per-pane tasks and does not expose them, so a test has
    /// to wait somehow — but waiting a fixed span makes the assertion depend on
    /// how loaded the machine is, and eleven suites run in parallel here. This
    /// polls a predicate with a generous deadline, so a slow machine takes longer
    /// and a fast one takes less, and neither changes the verdict.
    private func settle(
        until condition: @MainActor () -> Bool = { true },
        ...
        XCTFail("timed out waiting for the comparison to settle", file: file, line: line)
    }

    /// A pane has reached a terminal state (done or error).
    private func settled(_ model: NativeCompareModel, _ paneID: String) -> Bool {
        guard let status = model.runs[paneID]?.status else { return false }
...
private final class CompareClient: NativeCompareSending, @unchecked Sendable {
    ...
    func cancelGeneration(id: String, for accountID: AccountID) async throws -> Bool {
        ...
        // is pushed into a synchronous helper rather than held across a
        // suspension — the same shape `RecordingClient` uses.
        let continuation = recordCancel(id)
    ...
    private func recordCancel(
        _ id: String
    ) -> AsyncThrowingStream<NativeChatServerEvent, any Error>.Continuation? {
