...
Status: **callback hotfix deployed; sync rollout repaired and rehearsed on disposable PostgreSQL, pending production promotion and exact-candidate UI acceptance**.
...
- Production PR `#11` merged as `b283c91` and started deployment run `29520740833`. This is the narrow callback allowlist/handoff change only; it does not deploy the change-feed schema.
...
Remaining gates: rerun all Swift tests and the real-account one-window callback after the Mac is unlocked; promote and probe bootstrap/change/mutation endpoints; complete web↔Mac mutation scenarios; and obtain a valid Developer ID Application identity plus notarization credentials before publishing an automatic production update.
