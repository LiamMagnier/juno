...
## 2026-07-16 V3 callback isolation and migration rehearsal

Status: **callback and V3 sync server rollout deployed; exact-candidate Mac UI acceptance and Apple release signing remain pending**.

- Native commit `b0c4edb` gives Release builds the isolated reverse-domain callback `com.liammagnier.juno://auth/callback`, gives Debug builds a separate bundle identifier and callback, routes incoming URLs to an already-open SwiftUI scene, and stores the five-minute PKCE request in Keychain so a callback can survive a process relaunch. Build 28 compiled as a universal arm64/x86_64 Release app. A clean command-line build-for-testing and direct test-bundle run passed all 34 Swift tests in 8 suites; test-only commit `b927f7f` makes callback registration validation independent of a UI-hosted runner.
- Web commit `12ae3a8` accepts both build-27's legacy callback and the isolated V3 callback, then hands off to the exact server-validated redirect. Native-auth tests (7 tests on the integration branch, 4 on the production hotfix), TypeScript, Prisma validation, and both integration and current-main production builds passed.
- Production PR `#11` merged as `b283c91`; deployment run `29520740833` succeeded. This is the narrow callback allowlist/handoff change only.
- A clean production-history rehearsal first failed at `MessageVersion` after partially applying `20260716200000_account_change_log`, proving the migration and the active `db push` workflow were not safe for promotion. The migration is now transactional and idempotent, its triggers can be re-applied, and deployment baselines the historical `db push` state exactly once before switching permanently to `prisma migrate deploy`.
- A second disposable PostgreSQL 17 rehearsal applied all 22 current production migrations, seeded a user/settings/project/conversation/message baseline, performed the one-time schema/history convergence, ran the account-change script twice, and then applied the remaining migration. Conversation, settings, message, message-version, and delete events produced ordered change rows; a second conversation update advanced revision 1 to 2 without duplicate triggers.
- The endpoint security review found and fixed a cross-account reference gap in `conversation.update`: `projectId` and `folderId` are now independently ownership-checked before an owned conversation can be linked. The disposable API probe rejects a foreign project with 404 while preserving the conversation revision.

- Production PR `#12` merged as `37c3690`; deployment run `29521990017` succeeded. Production baselined four historical `db push` migrations once, reports all 23 migrations current, and restarted the backend, relay, and scheduler online. Unauthenticated live probes of bootstrap, changes, and mutations now return versioned 401 responses rather than 404.
- The exact production-branch API passed disposable signed-bearer acceptance: bootstrap, mutation, replay, cursor changes, foreign-reference rejection, owned-reference update, and revision advancement. Query-plan probes used the expected account/cursor and entity/revision indexes.
- Build 28 was ad-hoc signed and packaged as `Juno-3.0.0-alpha.6-unsigned.dmg`; `hdiutil verify` passed and its SHA-256 is `a167587a642694ac5476bd1c14af7885cb74a58afab1ba04ba7ae017deb00abc`. GitHub prerelease `v3.0.0-alpha.6` targets native commit `b0c4edb`. It remains a manual evaluation artifact, not an automatic production update.

Remaining gates: unlock the reference Mac; prove the installed build-28 real-account one-window callback, initial sync, relaunch, accessibility and web↔Mac mutation scenarios; then obtain a valid Developer ID Application identity plus notarization credentials before publishing an automatic production update.
