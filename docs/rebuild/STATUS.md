...
- Native commit `b0c4edb` gives Release builds the isolated reverse-domain callback `com.liammagnier.juno://auth/callback`, gives Debug builds a separate bundle identifier and callback, routes incoming URLs to an already-open SwiftUI scene, and stores the five-minute PKCE request in Keychain so a callback can survive a process relaunch. Build 28 compiled as a universal arm64/x86_64 Release app. The Swift test runner could not materialize while the reference Mac was locked, so the complete Swift test count must be rerun after unlock.
...
Remaining gates: unlock the reference Mac; rerun all Swift tests; install build 28 and prove the real-account one-window callback, initial sync, relaunch, accessibility and web↔Mac mutation scenarios; then obtain a valid Developer ID Application identity plus notarization credentials before publishing an automatic production update.
