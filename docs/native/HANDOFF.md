...
> **Current workspace snapshot (2026-08-06):** the active worktree is
> `/Users/liammagnier/Documents/workspace/juno`. The current JunoMac Code path
> includes the local web preview dock, model routing, Computer Use, MCP,
> hooks/skills, isolated sub-agents/worktrees, and Cloud/Remote target discovery
> plus task dispatch. The historical resume paths and “not implemented” notes
> below are retained for provenance; use `docs/native/RELEASE.md` and the source
> gates for current release status.
