...
- The active checkout has no `native/` directory, Swift source, Swift package,
  Xcode project, entitlement file, privacy manifest, or native CI job.
...
| Native PKCE, access/refresh, logout, device list and revocation | **Implemented** in `/api/v1/auth/*`, `src/lib/native-auth*.ts`, and Prisma native-session models | **Partial**. Secure server flow exists, but callback URI sources disagree and no native `TokenRefreshCoordinator` exists | **Missing** | **Missing** | Core JWT/validation tests only; no database-backed rotation/reuse/concurrency suite | Resolve callback contract, implement Keychain and single-flight refresh, add integration tests |
...
| Account synchronization | **Implemented** server bootstrap, cursor pages, SSE wakeups, hydration, revisions, tombstones, compaction and idempotent mutations | **Partial**. No native transactional database, migration layer, outbox, conflict resolver, reconnect actor or cache wipe implementation exists | **Missing** | **Missing** | Pure protocol/mutation tests exist; no Web-to-Swift contract or crash/offline E2E suite | Build shared storage/sync packages and run mandatory convergence scenarios |
| Global search | **Partial**. Current Web palette searches only chat titles and project names | **Missing** protected native index for decrypted text, files, artifacts, tasks and Code data | **Missing** | **Missing** | No native search-quality or privacy tests | Implement local protected index, filters, recents and wipe rules |
| Native design system, motion, accessibility and localization | Web semantic tokens, coral accent, warm surfaces, dot/ASCII signature and flat transcript are **Implemented** references | **Missing** Swift semantic tokens, String Catalogs, native motion/accessibility fallbacks | **Missing** | **Missing** | Web accessibility mechanisms exist; no native audits | Build `JunoDesignSystem` with VoiceOver, Dynamic Type, contrast and motion coverage |
| macOS desktop shell | Web shell is a behavioral reference only | Not applicable until native shell exists | **Missing** | N/A | No Xcode build or UI tests | Create an independent macOS project using native desktop conventions |
...
| Native distribution and updates | Web deploy and a legacy DMG download are **Implemented** artifacts | **Missing** reproducible signing, notarization, TestFlight/App Store, privacy manifest, dSYM and native release CI | **Missing source pipeline** | **Missing source pipeline** | No native Release archive checks | Add separate native CI/release jobs and verify signed artifacts before publication |
...
|---|---|---|---|---|---|---|---|
| `/api/v1/auth/token` | POST | No | Grant exchange | Yes | Authorization code consumed atomically | Core helpers only | Minimal token DTO / none |
| `/api/v1/auth/refresh` | POST | No | Refresh grant | Yes | Rotating family with reuse revocation | Core helpers only | Minimal token DTO / none |
...
Parity is not complete until both independent native projects build in Debug and
Release and the mandatory Web/Mac/iPhone scenarios pass: cross-surface creation
