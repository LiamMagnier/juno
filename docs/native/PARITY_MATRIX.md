...
- Native CI, production persistence/bootstrap composition, signed release inputs and
...
| Account synchronization | **Implemented** server bootstrap, cursor pages, SSE wakeups, hydration, revisions, tombstones, compaction and idempotent mutations | **Partial**. Existing bootstrap is connected through a refresh-aware bearer client and validates account/contract/cursors; tested cursor/outbox primitives exist. Durable SQLite/migrations, reconnect, conflict UI and compaction recovery remain | **Missing integration** | **Missing integration** | 12 Sync tests pass; no Web-to-Swift or crash/offline E2E suite | Add durable adapters and run mandatory convergence scenarios |
...
| macOS desktop shell | Web shell is a behavioral reference only | Independent project and shared package boundary exist | **Partial**. Native split-view and real auth gate build in Debug/Stable; feature flows are placeholders | N/A | Debug/Stable, 2 unit tests and auth-gate UI test pass | Compose bootstrap/storage, then migrate real desktop feature slices |
| iOS/iPadOS adaptive shell | Web shell and reference screenshots are behavioral references only | Independent project and shared package boundary exist | N/A | **Partial**. Adaptive split-view and real auth gate build in Debug/Stable simulator; feature flows are placeholders | Debug/Stable, 2 unit tests and auth-gate UI test pass | Compose bootstrap/storage, then implement sidebar/chat/search feature slices |
