...
## D-014 — Client-side HEIC transcoding rather than a widened server allowlist

- Context: iPhone cameras and the photo library return HEIC/HEIF by default, but `/api/upload` accepts only `image/png`, `image/jpeg`, `image/webp` and `image/gif`, and `sniffImageMime` verifies the real type from magic bytes and stores the *detected* type, never the client-declared one. A HEIC upload therefore fails the magic-byte check with 415 even if `image/heic` were added to `IMAGE_MIME`, because the sniffer has no HEIF branch.
- Decision: transcode HEIC/HEIF to JPEG on the Apple client before upload. Normalize orientation to `.up` so the pixel data no longer depends on an EXIF tag, strip metadata that is not needed for rendering, preserve the source dimensions unless an existing size limit forces a resize, and validate that the encoded result is a decodable JPEG before it is handed to the uploader. Surface processing progress and failures in the composer.
- Reason: the server's never-trust-client-MIME invariant is a deliberate stored-XSS and phishing control (non-images are stored as `application/octet-stream` and forced to download). Widening the allowlist would require a new HEIF sniffer branch and would extend that trust boundary to a container format with a substantially larger parser surface, for no user-visible benefit — the web client already uploads JPEG. Transcoding keeps the entire change on the client, where it can be tested without touching a security-relevant server path.
- Consequences: `sniffImageMime` is left unchanged and the client-supplied MIME type remains untrusted. The client owns transcode failure as a real, surfaced error state rather than an upload rejection the user cannot interpret. Live Photos and depth/HDR auxiliary data are discarded, matching web behaviour. Any future format the backend genuinely supports must still be added to both the allowlist and the sniffer, together.
- Files: `src/lib/uploads.ts` (unchanged, referenced), `src/app/api/upload/route.ts` (unchanged, referenced); client transcoding to be implemented in the composer attachment pipeline under Block 2.
- Status: decided; implementation pending in Block 2.

## D-015 — The backfill migration ships from `origin/main`, never from a feature branch

- Context: `prisma/migrations/20260721120000_backfill_entity_revisions/migration.sql` exists in two forms. `agent/juno-code-remote-backend` (and the other stacked feature branches) carry a 44-line version whose 22 NULL literals are **bare `NULL`**. `origin/main` carries a 44-line version whose same 22 literals are **`NULL::timestamp`**. The files are otherwise identical — same statements, same order, same length. Verified 2026-07-22 by comparing the branch worktree against `origin/main`: the branch has 0 occurrences of `NULL::timestamp` and 22 bare-`NULL` lines; `origin/main` has 22 `NULL::timestamp` and 0 bare-`NULL` lines.
- Decision: at integration, take this file **verbatim from `origin/main`**. Never deploy the feature-branch copy, and never resolve a conflict on this path by keeping "ours".
- Reason: the backfill inserts those literals through an `INSERT ... SELECT` over a `UNION ALL` chain. A bare `NULL` carries no type, so Postgres cannot infer the target column type for the `deletedAt` position and the statement fails. The `origin/main` version is the one that was actually deployed; the branch version is the earlier form that already failed once. Because the branch and main versions have the same name and length, a careless merge resolution looks clean and still ships the broken file.
- Consequences: any merge or rebase that touches this path must be resolved toward `origin/main`. This is a content rule, not an ordering rule — see the separate hazard for `20260719120000_remote_code_sessions`, whose timestamp sorts *before* this already-applied migration and which must not be renamed, applied or deployed reflexively.
- Files: `prisma/migrations/20260721120000_backfill_entity_revisions/migration.sql` (do not modify on any feature branch).
- Status: verified and recorded; enforced at PR #19 integration.
