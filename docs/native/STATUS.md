...
- Juno Code macOS integration is active in the unified `JunoMac` shell through
  `JunoMacCodeView` and the authenticated `NativeChatRequestSending` transport.
  The current workbench includes model-catalog routing across supported
  providers, approvals/recovery, MCP, hooks/skills, isolated sub-agents and
  worktrees, Computer Use, interactive terminal output, review/restore, and a
  real local web preview dock that discovers and supervises the workspace dev
  server. Cloud/Remote creation is also wired through the authenticated native
  task client: the session sheet discovers repositories/devices/workspaces,
  validates the target, dispatches a server-owned task, and reports failures
  without fabricating a local transcript.
...
- Unified JunoMac does not yet contain the full live remote task history/event
  browser that Desktop Code Studio provides; queued Cloud/Remote tasks remain
  server-owned and are monitored through the native task surface. This is a
  product-surface parity item, not a missing dispatch route.
...
The implementation work is now in the verification/publication phase: run the
validated source from a clean release tag, obtain the Developer ID/notarization
and App Store Connect inputs, then execute signed release checks. The unified
JunoMac composer can dispatch Cloud/Remote tasks and now includes the same live
remote task ledger as Desktop Code Studio: active/recent history, cursor-based
event streaming, reconnect recovery, approvals, cancellation, and pull-request
links. The local preview is also a one-click toolbar action and auto-starts a
contained workspace dev server when a safe command is discovered.

Open first:

1. `docs/native/RELEASE.md` (signing, notarization, and clean-source gates)
2. `docs/native/JUNO_CODE_HANDOFF.md` (current production limitations)
