 ...
 *
 * It is now a destination in its own right rather than a tab inside Work, which
 * is what the subject was always owed: permissions govern every delegated run
 * in the account, and a run is no longer something that happens in a separate
 * place (docs/design/TWO_PRODUCTS.md §2). `/work/permissions` and `/work/hosts`
 * both still answer and both land here.
 *
  ...
  return (
    <section className="mt-8">
      ...
      <p className="mt-4 text-ui text-muted-foreground">
        ...
        to another. Anything waiting for a decision right now is in the{" "}
        <Link href="/chat" className="underline underline-offset-4 hover:text-foreground">
          Needs you
        </Link>{" "}
        fold at the top of the sidebar — it is there on every page in the product as soon as a run
        stops for you — and a task’s own decisions are listed in its conversation under Progress.
      </p>
    </section>
  );
}
