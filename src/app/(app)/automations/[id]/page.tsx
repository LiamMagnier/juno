          ...
          <WorkList>
            <ul className="space-y-0.5">
              {runs.map((run) => (
                <li key={run.id}>
                  {/*
                    Through `/work/<sessionId>`, deliberately, and it is the one
                    URL under `/work` that is a live resolver rather than a
                    legacy redirect. A run row carries a session id and nothing
                    else; turning one into the conversation it writes into is a
                    per-account lookup only the server can do
