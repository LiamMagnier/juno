  ...
  if (!model) {
    return (
      <div className="flex w-full shrink-0 snap-start items-center border-l border-border/50 bg-card/60 p-5 md:w-64">
        <p className="text-xs text-muted-foreground">Hover a model to compare intelligence, speed, context, and cost.</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex w-full shrink-0 snap-start flex-col overflow-y-auto border-l border-border/50 bg-card/75 p-4 md:w-64">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-base font-semibold leading-tight tracking-tight">Auto Router</h3>
        ...
        <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
          Routes each message to the <span className="font-medium text-foreground">optimal model</span> and{" "}
        ...
        <ul className="mt-4 space-y-2 text-label leading-snug text-muted-foreground">
          <li className="flex gap-2"><span className="font-mono text-primary font-bold">1</span> Everyday prompt → Fast models · Instant</li>
          <li className="flex gap-2"><span className="font-mono text-primary font-bold">2</span> Coding & analysis → Mid tier · Balanced</li>
          <li className="flex gap-2"><span className="font-mono text-primary font-bold">3</span> Deep reasoning → Flagship · Deep thinking</li>
        </ul>
          ...
          <div className="space-y-2 border-t border-border/40 pt-2.5">
            ...
            {hasLiveBenchmark(model) && (
              <p className="font-mono text-micro text-muted-foreground/60">
                Scores by{" "}
                <a href="https://artificialanalysis.ai" target="_blank" rel="noreferrer" className="underline decoration-dotted hover:text-muted-foreground">
                  Artificial Analysis
                </a>
              </p>
            )}
          ...
          {options.length > 1 ? (
            <div className="space-y-2">
              <div className="flex items-baseline justify-between">
                <span className="font-mono text-micro text-muted-foreground">Thinking depth</span>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-primary/15 text-primary ring-1 ring-primary/30 shadow-xs" : "text-muted-foreground hover:bg-accent/60 hover:text-foreground",
        ...
        >
          ...
          <div className="flex items-center gap-2.5 w-full pr-6">
            ...
            <div className="min-w-0 flex-1">
              <span className="block truncate text-xs font-semibold tracking-tight">{m.name}</span>
              <span className="block truncate text-caption text-muted-foreground">{PROVIDERS[m.provider].label.split(" · ")[0]}</span>
            </div>
          </div>
        ...
        {features.billing && plan !== "MAX" && plan !== "OWNER" && (
          ...
          >
            <span className="flex items-center gap-2 text-xs font-semibold text-primary">
              <Zap className="size-3.5 fill-current" /> Upgrade to unlock frontier models
            </span>
            <span className="rounded-md bg-primary px-2 py-0.5 text-micro font-semibold text-primary-foreground shadow-xs">Upgrade</span>
          </button>
        )}
          ...
          <div className="flex w-full shrink-0 snap-start md:w-auto md:flex-1 md:shrink">
            ...
            <div className="flex w-14 shrink-0 flex-col items-center gap-1.5 overflow-y-auto border-r border-border/50 bg-muted/25 p-2">
              <RailButton active={filter === "all"} title="All models" onClick={() => setFilter("all")}>
                <LayoutGrid className={cn("size-4.5", filter === "all" ? "text-primary" : "text-muted-foreground")} />
              </RailButton>
                ...
                {providerFilter && !filterConfigured ? (
                  <div className="flex h-full flex-col items-center justify-center gap-2 px-6 py-10 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Add <span className="font-mono text-primary font-semibold">{PROVIDERS[providerFilter].apiKeyEnv}</span> in Settings to enable these models.
                    </p>
                  </div>
                    ...
                    return (
                      <div key={g.key} className="space-y-1.5">
                        <div className="flex items-center gap-1.5 px-1 pt-0.5">
                          ...
                          <span className="text-caption font-medium text-muted-foreground/80">{g.label}</span>
                        </div>
                            ...
                            <button type="button" onClick={() => select(AUTO_MODEL_INFO)} className="flex w-full items-start gap-2.5 text-left">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-1.5">
                                  <span className="truncate text-xs font-semibold">Auto</span>
                        ...
                        {legacyItems.length > 0 && (
                          <div className="mt-2">
                            <details key={q ? "open" : "closed"} open={!!q} className="group/legacy rounded-card border border-border/40 bg-muted/10 overflow-hidden">
