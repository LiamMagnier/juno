...
>(({ className, ...props }, ref) => (
  <CheckboxPrimitive.Root
    ref={ref}
    className={cn(
      "surface-inset peer relative flex size-[18px] shrink-0 items-center justify-center rounded-xs border border-input transition-[background-color,border-color,box-shadow] duration-fast ease-out-soft before:absolute before:left-1/2 before:top-1/2 before:size-6 before:-translate-x-1/2 before:-translate-y-1/2 before:content-[''] coarse:before:size-11 hover:border-foreground/60 disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transition-none data-[state=checked]:border-primary/80 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground data-[state=checked]:[box-shadow:var(--shadow-pressed),var(--glow-primary)] data-[state=indeterminate]:border-primary/80 data-[state=indeterminate]:bg-primary data-[state=indeterminate]:text-primary-foreground data-[state=indeterminate]:[box-shadow:var(--shadow-pressed),var(--glow-primary)]",
      className
    )}
    {...props}
  >
