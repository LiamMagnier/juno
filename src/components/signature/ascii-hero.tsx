...
export function AsciiHero({ className }: { className?: string }) {
  ...
  return (
    <svg
      ...
      className={cn("text-primary motion-safe:animate-drift", className)}
      aria-hidden="true"
    >
      {/* central coral sun */}
      ...
      {rings.flatMap((ring) =>
        Array.from({ length: ring.count }).map((_, i) => {
          ...
          return <circle key={`${ring.radius}-${i}`} cx={x.toFixed(2)} cy={y.toFixed(2)} r={r} fill="currentColor" opacity={ring.op} />;
        })
      )}
    </svg>
  );
