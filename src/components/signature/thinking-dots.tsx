...
export function ThinkingDots({ className }: { className?: string }) {
  return (
    <span
      className={cn("relative inline-grid size-4.5 shrink-0 grid-cols-3 gap-[3px] text-muted-foreground", className)}
      aria-hidden="true"
    >
      {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((gridIndex) => {
        const sequenceIndex = MATRIX_SEQUENCE.indexOf(gridIndex);
        return (
          <span key={gridIndex} className="relative block size-1">
