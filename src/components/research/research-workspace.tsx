...
function ResearchDetail({ runId }: { runId: string }) {
  ...
  return <div className="research-enter">
    ...
    {canSteer && <form onSubmit={submitSteer} className="mt-7"><label htmlFor="research-steer" className="mb-2 block text-ui font-medium">Adjust the focus</label><div className="flex items-center gap-2 rounded-field border border-border bg-card p-2"><input id="research-steer" value={steer} disabled={busy} maxLength={400} onChange={event => setSteer(event.target.value)} placeholder="Add a constraint or a source URL…" className="min-w-0 flex-1 bg-transparent px-2 py-2 text-ui outline-none focus-visible:ring-2 focus-visible:ring-ring" /><Button type="submit" size="icon-sm" aria-label="Update research focus" className="rounded-sm" disabled={busy || steer.trim().length < 3}><ArrowUp className="size-4" /></Button></div>{inputError && <p role="alert" className="mt-2 text-ui text-destructive">{inputError}</p>}</form>}
