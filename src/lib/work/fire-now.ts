...
export type FireOutcome =
  ...
  | { outcome: "refused"; reason: FireRefusal; message: string; selection?: TargetSelection };
...
export async function fireScheduleNow(input: FireNowInput): Promise<FireOutcome> {
  ...
  const created = await createRun({
    ...
    permissionPolicy,
    // The routine's own figures, narrowed against this account's plan ceiling.
    // Zero on these columns means "no ceiling of the routine's own" to
    // `budgetExceeded`, and `narrowestBudget` skips the zeros rather than
    // clamping to them.
