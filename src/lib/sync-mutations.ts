...
export const mutationOperationSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("project.update"), entityId: z.string().min(1).max(200), name: z.string().trim().min(1).max(160).optional(), instructions: z.string().optional() }).strict(),
