...
/**
 * True when a candidate is covered by a suppression note (exact match or
 * containment either way). Containment is intentionally generous: "forget my
 * address" must also block "the user lives at 12 Rue Oberkampf", and a
 * suppression that is too broad only costs a fact the user already said they
 * did not want.
 */
export function isSuppressedBy(candidate: string, suppressions: readonly string[]): boolean {
  const normalizedCandidate = normalizeFact(candidate);
  if (!normalizedCandidate) return true;
  return suppressions.some((suppression) => {
    const normalizedSuppression = normalizeFact(suppression);
    return (
      normalizedSuppression.length > 0 &&
      (normalizedCandidate === normalizedSuppression ||
        normalizedCandidate.includes(normalizedSuppression) ||
        normalizedSuppression.includes(normalizedCandidate))
    );
  });
}
