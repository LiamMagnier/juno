...
/*
 * Signed capabilities for the Cloud Juno Code runner (GitHub Actions).
 *
 * A cloud task is dispatched to an untrusted CI VM that runs arbitrary agent
 * bash. Two distinct, single-secret-signed credential KINDS keep the blast
 * radius tiny:
 *
 *  - EXCHANGE code ("ccx_", ~3 min): the ONLY credential that rides the public
 *    workflow_dispatch input. It authenticates exactly one call — GET
 *    runner-context — which single-uses it (see runnerClaimedAt) and hands back
 *    a task token + clone token. It cannot post events, drive the agent proxy,
 *    or do anything else. Once the runner-context handoff is consumed the code
 *    is INERT even if an attacker later reads it out of /proc.
 *
 ...
 *
 * Both are HMAC-SHA256 over a compact { taskId, exp, kind } payload keyed by
 * CLOUD_CODE_SECRET (never shipped to the runner). The `kind` is part of the
 * SIGNED payload and is cross-checked on read, so a task token can never be
 * presented where an exchange code is required (or vice-versa) — even though a
 * single secret signs both.
 *
 * Format:  <prefix><base64url(payload)>.<base64url(hmac)>
 *   payload = JSON { taskId, exp, kind }   exp = unix SECONDS
 *
 * The audience is the EXACT taskId: a credential minted for task A never
...
/** Exchange code: the one-time dispatch input, redeemed once at runner-context. */
export const CLOUD_CODE_EXCHANGE_PREFIX = "ccx_";
 ...
 * (a `cct_` token read as an exchange code, or vice-versa, fails here). Does not
 * bind to a caller-supplied taskId — used where the taskId is unknown up front
 * (the provider proxy). For an audience-bound check use verifyTaskToken /
 * verifyExchangeCode.
 */
function read(prefix: string, expectedKind: TokenKind, token: string): string | null {
  ...
  // Kind is signed AND cross-checked: this is what makes the two credentials
  // non-interchangeable despite sharing one secret.
...
/** Audience taskId of a valid, unexpired task token, else null. Rejects an
 *  exchange code presented as a task token. */
...
/** Mint a one-time exchange code scoped to exactly `taskId`, valid for `ttlMs`. */
export function mintExchangeCode(taskId: string, ttlMs: number = CLOUD_CODE_EXCHANGE_TTL_MS): string {
  return mint(CLOUD_CODE_EXCHANGE_PREFIX, "exchange", taskId, ttlMs);
}

/** Audience taskId of a valid, unexpired exchange code, else null. Rejects a
 *  task token presented as an exchange code. */
export function readExchangeCode(token: string): string | null {
  return read(CLOUD_CODE_EXCHANGE_PREFIX, "exchange", token);
}

/** True iff `token` is a valid exchange code whose audience is EXACTLY `taskId`. */
export function verifyExchangeCode(token: string, taskId: string): boolean {
  return audienceMatches(readExchangeCode(token), taskId);
}
