...
const WORKER_SOURCE = readFileSync(new URL("../scripts/sweep-import-runs.ts", import.meta.url), "utf8");
...
test("the recovery worker wraps async startup for the production CJS runner", () => {
  assert.match(WORKER_SOURCE, /async function main\(\)/);
  assert.match(WORKER_SOURCE, /void main\(\)\.catch/);
  assert.doesNotMatch(WORKER_SOURCE, /^await\b/m);
});
